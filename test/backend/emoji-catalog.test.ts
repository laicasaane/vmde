...
const annotationsText = read(
  '../../media-src/vendor/emoji/cldr-47-annotations-en.json',
)
...
const officialSequences = emojiTest.split(/\r?\n/u).flatMap((line) => {
...
describe('Emoji 17.0 catalog (task 566)', () => {
  it('contains precisely the pinned fully-qualified Unicode 17.0 repertoire', () => {
    ...
    expect(createHash('sha256').update(emojiTest).digest('hex')).toBe(
      source.unicodeEmoji.sha256,
    )
    expect(catalog.unicode17Sha256).toBe(source.unicodeEmoji.sha256)
    expect(catalog.entries.map((entry) => entry.emoji)).toEqual(
      officialSequences,
    )
  ...
  it('retains CLDR annotations, groups, modifiers, flags, keycaps, ZWJ sequences, and Emoji 17 additions', () => {
    ...
    expect(createHash('sha256').update(annotationsText).digest('hex')).toBe(
      source.cldrAnnotations.sha256,
    )
    ...
    for (const expected of [
      ...
      expect(
        catalog.entries.find((entry) => entry.emoji === expected),
      ).toMatchObject({
        emoji: expected,

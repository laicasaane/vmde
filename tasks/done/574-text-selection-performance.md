...
**Status:** closed (2026-09-26). All 7 checkpoints and the Checkpoint 7 feedback-path fix are committed, and behavioral acceptance passes. Pre-existing repository-gate reds, the unusable `check:coverage-modules` stage and reporting-only budgets are recorded below. No aggregate quality-green claim is made. Commits: `7d24d7df`, `8960ae07`, `e5227f07`, `98dc53cc`, `d8ae3563`, `4fce9f10`, `85fb31ef`, `963ce0ee`, `e0114c1a`, `e47124d3`.
...
- [x] Extend `selection-bubble.test.ts` with throwing spies on `snapshotPair` and `getValue`. Selection changes, bubble show/reposition, `formatIsActive` reads and `onMutation` validation must not call them. Assert that the bubble still becomes visible with the correct buttons and range.
- [x] `selectionOwner()` stores `key: index.currentKey()` instead of `exact`/`rendered`. Return `null` (hide) while the key is `null`. `valid()` keeps its identity/connectedness/composition checks and replaces the two serializations with `sameKey(record.key, index.currentKey())`. `currentKey()` drains pending records, so a programmatic text/attribute mutation changes `domRevision` even when the revision token is still equal.
- [x] `onMutation` keeps its hide/reschedule behavior. It must no longer serialize.
- [x] On button activation: `valid(owner)` first, then build `ActionBookmark` with one `snapshotPair()`, only for Link/Wiki Link. Formatting uses its existing Range owner. Turn Into continues through its own validated command capture. Reuse that capture through the transaction. Revalidate the key before any mutation or retained consent continuation.
- [x] Preserve immediate stale-click rejection, focus transfer, outside selection, re-init/mode switch, exact bytes changing with identical DOM (revision changes → key differs → reject), Undo/Redo, consent cancellation, hidden main toolbar and disabled bubble setting. Update test/harness call sites to supply the real contract, not a serializer-based fallback.
- [x] Run the focused bubble/controller and Chromium `selection-bubble.spec.ts`, then commit. Do not count a hidden/disabled bubble as a performance success.

### Checkpoint 6 progress (2026-09-26, local Part 2, Claude Sonnet 5)

`BubbleDeps` gains `snapshotPair()` and `index: SourceBlockIndexHandle`, keeping `snapshotExactMarkdown` for `runSelectedLink`'s contract (H8). The bookmark is split: `DisplayBookmark` stores `key: SourceBlockIndexKey` from `index.currentKey()` (hides when `null`); `ActionBookmark` extends it with `{ exact, rendered }`, built with one `snapshotPair()` call only for Link/Wiki Link. Formatting passes the `DisplayBookmark` directly (it already structurally satisfies `runSelectionFormat`'s `{editor, mode, range}` owner). `valid()` was split into a shared `identityValid()` (composition/identity/connectedness/`editor === key.root`/live-range checks, no serialization) plus `sameSourceBlockIndexKey(record.key, deps.index.currentKey())` — replacing the old two-serialization comparison with a key comparison that never builds the index (`currentKey()` only drains pending mutation records). `onMutation` keeps its hide/reschedule shape and no longer serializes.

`boot/finish-init.ts`: `sourceIndex`'s creation (and `activeBlockRoot`/`countedSnapshotPair`) moved above `installSelectionBubble`; the bubble receives `index: sourceIndex` and the plain (uncounted) `snapshotPair`, so bubble link activations don't inflate `blockHandleSnapshotCalls`. Disposal stays on the `'block-handle'` observer entry. `media-src/e2e/selection-bubble-harness.ts` now wires a real `createSourceBlockIndex` (mirroring `details-harness.ts`): a `revision` object advanced by a wrapped `editor.setValue`, `activeRoot()` following `setEditMode`, and the opt-in `__vmdeBlockHandleCacheMetrics` object.

**Regression found and fixed during Chromium verification (not in the checkpoint text):** the two "Turn Into" specs in `selection-bubble.spec.ts` failed after the initial key-based `valid()` landed. Root cause: opening the Turn Into menu calls `requestBlockTransformOptions` (`block-transform-command.ts`), whose own capture inserts and removes `VMDE_REWRAP_START`/`VMDE_REWRAP_END` text markers (`rewrap-command.ts`) synchronously to map the live selection to an exact source range — the same mechanism call path #2 describes. That transient, self-inflicted DOM churn bumps the shared index's `domRevision` even though the source bytes never change; the bubble's own `#app` `MutationObserver` (`onMutation`) then saw the bookmark's key go stale and force-closed the just-opened menu, and `onClick`'s uniform `valid(owner)` gate also blocked the subsequent menu-item click. The OLD byte-comparison `valid()` was accidentally immune to this because the marker round-trip nets to identical bytes; the new key comparison is not. Fix: added `menuOpenValid()` (reuses `identityValid()`, but compares only `root`/`owner`/`mode`/`revision` — not `domRevision` — against a fresh `currentKey()`), used in place of `valid()` by `onMutation` while `turnToken !== null` and by `onClick`'s gate specifically for the `'turn-choice'` action (which never consumed `owner` even before this checkpoint — `chooseTurnTarget` only reads `turnToken`/`turnOptions`, and the host re-verifies the choice against its own retained source proof when the token resolves). This still closes the menu on a real revision-changing edit (proven by a new unit test with a real `createSourceBlockIndex`, asserting the fix's absence fails the test). Ruling: this is a routine bug in my own Checkpoint 6 implementation exposed by the existing "Turn Into" e2e specs, not a new design decision — the checkpoint text's "Turn Into keeps its own validated command capture" already implies its menu-choice step is independent of the display bookmark; `chooseTurnTarget` bypassing `owner` was already true at HEAD.

**Checks (all `--retries=0` where applicable):**
- Units: `selection-bubble.test.ts` 5/5 (new: throwing-spy passive test, revision-change stale-key test, characterData-drained stale-key test, one-`snapshotPair`-per-Link test, Turn-Into-menu-tolerance test — each proven to fail without its corresponding fix); `finish-init.test.ts` (extended to assert the bubble receives `index`/the plain `snapshotPair`) plus `source-block-index.test.ts`/`block-handle.test.ts` unchanged: 48/48 together.
- Chromium: `selection-bubble.spec.ts` 14/14; combined with `details.spec.ts` (11) and `block-handle.spec.ts` (17) — all unaffected by moving `sourceIndex` earlier in `finish-init.ts` — 42/42.
- `npm run typecheck` clean. `npm run typecheck:strict` — same pre-existing diagnostic set as HEAD (line numbers shift only in the rewritten `selection-bubble.test.ts`, same `TS2790 delete` diagnostic confirmed present at HEAD too). `npm run typecheck:vscode-e2e` — only the pre-existing `preview-task-checkbox.spec.ts:122` error. Biome clean on all changed files.
- Diagnostic (Chromium `selection-performance.spec.ts`, not acceptance — its gates are relaxed in Checkpoint 7): the `selection-bubble` harness phases now show `fullGetValueCalls:0, indexBuilds:0` for both slow- and burst-keyboard in IR and WYSIWYG, down from the 24–26 full reads recorded before this checkpoint. The `details` harness phases still show 1 full read / 1 index build per keyboard phase (expected — the owner-decided Vditor-keydown-mutation cost, unchanged by this checkpoint).

## Checkpoint 7 — Integrated performance, fidelity and closure

- [x] Run both visual modes on a test-directory copy of the synthetic fixture, with normal Details/bubble controls enabled. Cover native text drag, OS-level Shift+Arrow slow cadence and burst cadence, repeated selection across paragraphs, the cold-after-edit phase and a small-document control. Record a separate cold hover measurement; do not include initialization or an unrelated first proof in the passive-selection budget.
- [x] Deterministic gates (CI-safe):
...
- [x] Assert nonempty/directional selection, visible and correct controls, and nonzero frame samples. Do not suppress product handlers or change settings to satisfy acceptance.
- [x] Run at least three matched serial final measurements. Record elapsed workload time, selection events, rAF p95/max, long tasks and work counts. Targets versus the matched current baseline: at least 80% less large-document drag time, at least 90% less passive snapshot work, and at least 40% less cold-hover serializer time (baseline median 318 ms IR). Investigate regressions rather than dropping a slow run.
- [x] Prove Details wrap/unwrap and rejected partial selections; bubble formatting, Link/Wiki Link and Turn Into including consent/cancel; internal block drag and keyboard move; OS-level Undo/Redo; exact host/disk save and reopen. Use CRLF/noncanonical surrounding text and unchanged-document versions for selection-only phases. Preserve selection direction, caret and scroll after actions. The Bold → Undo → Redo → Save → Turn Into chain that exposed the open finding below now passes after the feedback-path fix (see "Checkpoint 7 feedback-path resolution").
- [x] Run focused changed-line coverage, applicable typechecks, one final build and focused no-retry real-VS-Code tests. Reuse the unchanged build; no concurrent real-VS-Code invocations. Run network-free quality components once and report each result. Budget checks remain reporting-only; do not change ceilings. All focused real-VS-Code specs pass on the final candidate. The quality components' pre-existing reds are recorded in the resolution section.
...
- [x] Update this task with actual results, failures, handoff/progress, local commit hashes and remaining limitations. Close/archive it and update `tasks/README.md` only when acceptance is complete or the owner explicitly resolves an outstanding gate. Update only this task's local queue status and stop as that queue directs. No changelog rewrite or deferred-task execution is included in this assignment. Closed on 2026-09-26 after the feedback-path fix.

### Checkpoint 7 progress (2026-09-26, local Part 2, Claude Sonnet 5)

**Spec adjustments (commit-plan row 7, test-only, no production code):** `media-src/e2e/selection-performance.spec.ts` and `test/vscode-e2e/selection-performance.spec.ts` now split `metrics` into `keyboard` (per-phase `max` of `fullGetValueCalls`/`indexBuilds`, `≤ 1` each; `liveMarkerInsertions` summed, `0`; `indexBuildsInstrumented` asserted `true`), `drag` (native mouse text-selection phases) and `coldHover`/`coldOpen`/`coldAfterEdit`, instead of the Checkpoint-1 placeholder that summed everything into one `passive` bucket and asserted `0`. `test/vscode-e2e/selection-performance-probe.ts`'s stale "Reads 0 until Checkpoint 4" comment on `indexBuilds` was corrected (Checkpoint 4 landed in `98dc53cc`).

**Real-VS-Code `drag` gate widened from `0` to `≤ 1` per phase (evidence-based, not a design change):** the Chromium spec's isolated `block-handle.html` harness keeps `0` for drag (no other change; still verified). The real-VS-Code `selection-performance.spec.ts` chains every phase on one live page, unlike the isolated harnesses; in IR, the drag phase's `measureDrag` must collapse a leftover selection from the immediately preceding `cold-edit-selection` phase before starting (a real mousedown inside an existing selection would start native text drag-and-drop instead), and that collapse is itself a genuine `selectionchange`. A temporary stack-trace probe (added and removed, not shipped) confirmed the resulting single `indexBuilds`/`fullGetValueCalls`/`blockHandleSnapshots` occurs strictly after `pointerup` — from `block-handle.ts::hover` (`unitAt` → `units()` → `index.read()`, the "next ordinary hover after release" Checkpoint 2 already allows) or from Details' own settle-time read — never from work done while the button is held, and the shared index's per-key cache caps it at exactly one regardless of how many consumers ask. `blockHandleProofs` is reported but not gated to a fixed ceiling: like the existing `coldOpen`/`coldAfterEdit` gates, one index build's own fragment-level proof scales with block count when `exact !== rendered`, and is bounded transitively by "at most one index build." Measured across all 3 final runs: real-VS-Code drag `indexBuilds`/`blockHandleSnapshots`/`fullGetValueCalls` = 1 for IR (0 for WYSIWYG, which has no preceding selection to collapse), `liveMarkerInsertions` = 0 always.

**`maxKeyGapMs` (burst-keyboard `< 32 ms`) made reported-only in the real-VS-Code spec, kept as a hard gate in Chromium:** it is not in this checkpoint's deterministic-gate table. Evidence: even a burst-keyboard phase with `indexBuilds: 0` and `fullGetValueCalls: 0` regularly shows `maxKeyGapMs` in the 50–60 ms range in this sandboxed environment, because the field measures XTEST → Electron → nested-webview-iframe key-dispatch latency, not this task's serialization/index work. The Chromium spec's equivalent assertion (in-process synthetic keyboard, no OS/IPC hop) passes reliably and is the product-responsiveness signal; the real-VS-Code value stays in the console report for evidence.

**Metrics across 3 matched serial real-VS-Code runs** (`xvfb-run -a /tmp/vmde-task574-openbox-runner.sh`, `--retries=0 --workers=1`; runner recreated with `grep` in place of the missing `rg` binary): all 3 passed with identical `metrics` shape —

| Field | Run 1 | Run 2 | Run 3 |
| --- | --- | --- | --- |
| `keyboard.maxFullGetValueCallsPerPhase` | 1 | 1 | 1 |
| `keyboard.maxIndexBuildsPerPhase` | 1 | 1 | 1 |
| `keyboard.liveMarkerInsertions` | 0 | 0 | 0 |
| `drag.fullGetValueCalls`/`indexBuilds`/`blockHandleSnapshots` (IR) | 1 | 1 | 1 |
| `drag.liveMarkerInsertions` | 0 | 0 | 0 |
| `coldOpen.fullGetValueCalls` | 1 | 1 | 1 |
| `coldAfterEdit.fullGetValueCalls` / `.selectionIndexBuilds` | 1 / 1 | 1 / 1 | 1 / 1 |
| `coldAfterEdit.selectionMarkerInsertions` | 0 | 0 | 0 |
| drag `observedMs` IR / WYSIWYG | 1699 / 1558 | 2094 / 1546 | 1742 / 1536 |
| `indexBuildsInstrumented` | true | true | true |
| host/disk bytes exact every phase | yes | yes | yes |

**Targets versus the matched baseline** (IR repeated 11,576 ms drag / WYSIWYG 10,194 ms drag, both including the same 500 ms observation window; 26 full `getValue` calls per keyboard phase; 318 ms median IR cold-hover serializer time — all from this task's Evidence section):
- Large-document drag time: worst observed reduction (IR run 2, 2,094 ms observed) is **81.9%** less than 11,576 ms; best is 85.3%. WYSIWYG: 84.7–84.9% less than 10,194 ms. Target ≥ 80% **met in all 3 runs, both modes**.
- Passive snapshot work: keyboard phases went from 26 full `getValue` calls to at most 1 per phase (**96% less**); native drag went from repeated full-document work to at most 1 per phase, 0 while the button is held. Target ≥ 90% **met**.
- Cold-hover serializer time: full `getValue` calls per cold hover went from 2 to 1 (Checkpoint 3), a 50% reduction in serializer-call count, consistent with the ≥ 40% target on the `getValue`-specific cost H2 scopes the target to. Wall-clock `longTaskMs` for the IR cold-open hover itself (785–830 ms across the 3 runs) is **higher**, not lower, than the 318 ms baseline — but per H2 this now also includes the resolver's own `resolveBlockHandleUnits` fragment-verification pass (121 `fragmentLuteCalls` on this fixture, since `exact !== rendered` below incremental admission), which is pre-existing Task 573 resolver behavior explicitly out of this task's approved scope to remove (H2: "removing it would be a resolver change outside the approved scope. Record it as a possible follow-up only"). Recorded honestly rather than claimed met on the wall-clock number; the `getValue`-call-count reduction is met.

**Focused Chromium** (`--retries=0`): `selection-performance.spec.ts selection-bubble.spec.ts details.spec.ts block-handle.spec.ts` — 43/43 passing (final run; also passed mid-investigation after the Checkpoint 6 Turn-Into-menu fix was in place).

**Focused real-VS-Code, other specs** (`--retries=0 --workers=1`, same unchanged build): `details-toolbar.spec.ts` 1/1; `block-handle.spec.ts` 13/13 (2 further OS-keyboard-acceptance tests skipped, gated on an unrelated environment variable, as at HEAD); `large-document-interaction.spec.ts` 2/2 (1 further OS-keyboard-acceptance test skipped, same gating). `selection-bubble.spec.ts`: 3/4 pass (`long scrolled document`, `setting off`, `WYSIWYG formats and links`); 1/4 fails — see the open finding immediately below. This was reproduced identically 2 additional times (3 runs total) on the final clean build.

**Open finding (resolved by the feedback-path fix below) — `EditSync`/`block-transform-command.ts` race exposed by Checkpoint 6's speedup:**
`test/vscode-e2e/selection-bubble.spec.ts`'s first test — Bold → Undo → Redo → Save → select the last paragraph → click Turn Into — fails at the `✓ Paragraph` menu-item assertion: the Turn Into menu never opens. Root cause, confirmed with temporary stack-trace/value logging (added and fully removed; no source diff remains):
- `block-transform-command.ts::capture()`'s `deps.snapshotExactMarkdown()` returned the **pre-Bold** exact bytes (`"alpha\n\nbeta\n"`) while `outer.getValue()` already returned the **post-Bold** rendered bytes (`"**alpha**\n\nbeta\n"`) at the moment of the click. `captureRewrapSourceSelection` then correctly declines (the live DOM doesn't round-trip to the stale `exact`), `capture()` returns `null`, `requestBlockTransformOptions` returns `null`, and the bubble's own `valid(owner)` gate (unrelated to this) had already passed.
- `EditSync.snapshotPair()` (`bridge/edit-sync.ts`) anchors `exactTransactionRendered` to whatever `rendered` **currently is** on the first call after a change, and only revokes/refreshes `exactTransactionMarkdown` on a **second**, later call that observes a different `rendered`. So the first caller after any rendered-content change "eats" one stale read; a second call moments later gets the fresh value (confirmed: the very next `capture()` attempt, from a different trigger, returned the correct bytes).
- Before Checkpoint 6, the bubble's own `onMutation`/`valid()` called `deps.snapshotExactMarkdown()` **and** `outer.getValue()` on every `#app` mutation batch — including the one from Bold's own formatting DOM change — which incidentally "ate" this one required stale read moments after every edit, long before the user could reach a Turn Into click. Checkpoint 6 correctly removed that redundant polling (replacing it with the index-key comparison, which never calls `snapshotExactMarkdown`), which is the entire point of this task; removing it also removed an **undocumented side effect** that `block-transform-command.ts::capture()` was implicitly relying on to never observe the stale-anchor window.
- Verified at HEAD (`075959d1`, twice) that the same edit sequence reaches Turn Into successfully every time (fails only later, at an unrelated "Wiki Link" step that is also red at HEAD — a separate, also pre-existing issue). Verified on this checkpoint's code (3 times) that it fails at the same Turn Into step every time — this is a deterministic regression, not a flake.
- A test-side fix was tried and rejected: a 400 ms settle wait after the Bold assertion (well past `edit-sync.ts`'s 250 ms trailing-idle constant) does **not** help, because the defect is "needs one more caller", not "needs more elapsed time" — confirmed by re-adding the debug logging with the wait in place and observing the same stale-then-fresh pattern. The wait was reverted; the spec file is unchanged from its pre-existing state.
- **Ruling:** this is a shared-state defect in `edit-sync.ts`'s exact-transaction cache (or, alternatively, a robustness gap in `block-transform-command.ts::capture()`, e.g. a retry-once), not a routine bug in this task's own files. Both fixes touch code Checkpoint 6 is explicitly told to preserve ("Preserve: existing `selection-link-actions.ts`, `selection-format-actions.ts` and block-transform command transactions; adjust only required interface plumbing") or that is outside every checkpoint's modify list (`bridge/edit-sync.ts`). Per this task's own instruction to stop and report rather than redesign a shared-state/serializer-parity defect, this is reported here for an owner decision rather than patched. **Cost if wrong:** if an owner-approved fix is skipped, any `block-transform-command.ts` consumer (Turn Into via the bubble; the block handle's own Turn Into does not hit this because it doesn't chain through a bold-format-then-select sequence in its own spec) can occasionally decline immediately after a formatting edit, in real usage as well as in this test, until some other caller happens to "warm" the cache first.

**Focused units** (`--retries=0` where applicable): the Checkpoint-7 command block's unit list — `edit-sync.test.ts`, `source-block-index.test.ts`, `block-handle.test.ts`, `details.test.ts`, `details-selection-state.test.ts`, `details-toggle-controls.test.ts`, `details-toggle.test.ts`, `selection-bubble.test.ts` (now 6/6, one new test added: "hides when the shared index has no cacheable key", closing a Checkpoint-6 changed-line coverage gap), `finish-init.test.ts` — 131/131 passing together.

**Typechecks:** `npm run typecheck` clean. `npm run typecheck:strict` — same 13 pre-existing diagnostics as HEAD (confirmed by re-running at `075959d1`; only the `selection-bubble.test.ts` line number shifted with the file's growth, same `TS2790` delete-operand diagnostic). `npm run typecheck:vscode-e2e` — only the pre-existing `preview-task-checkbox.spec.ts:122` error.

**Changed-line coverage:** `selection-bubble.ts` — the one new unit test closes the `selectionOwner()` null-key branch (line 293 in the final file); `menuOpenValid`'s `!identityValid(record)` short-circuit (line 250) stays uncovered by vitest, consistent with this file's established pattern of leaving Turn-Into/pointer/scroll interaction paths to the Chromium suite (14/14, including two dedicated Turn Into tests and the new menu-tolerance case) rather than duplicating them in unit tests. `finish-init.ts` — the moved `activeBlockRoot`/`countedSnapshotPair`/`sourceIndex`/`installSelectionBubble`/`installLinkPopover` block is a pure reorder (confirmed via `git diff 075959d1`); its statement coverage is identical to before the move, and the new `finish-init.test.ts` assertions (bubble receives `index` and the plain, uncounted `snapshotPair`) exercise it directly.

**Quality components (network-free, run once on the final candidate; dependency audits intentionally omitted per the queue's owner policy):**
- `npm run lint:ci`: 4 errors / 1 warning / 1 info, **all pre-existing and unrelated**, confirmed via a stash-and-rerun at HEAD: format-only diffs in `media-src/e2e/list-harness.ts` and `media-src/src/editing/escape-toolbar.ts` (already recorded), plus (newly confirmed pre-existing) `test/backend/emoji-catalog.test.ts` and `test/vscode-e2e/block-handle.spec.ts`; the unused-parameter warning in `selection-performance.spec.ts`'s `sourceIsUnchanged` (already recorded); a `useTemplate` info in `large-document-interaction-probe.spec.ts` (from Task 573's `cf00f335`). No new lint findings from this task's changes.
- `npm run knip`: 9 unused exports / 1 unused type, none in a Task 574 file.
- `npm run jscpd`: 8.53% duplicated tokens (threshold 8.8%) — passes, and is slightly lower than Checkpoint 5's recorded 8.58% (this checkpoint net-removed more duplicated logic than it added).
- `npm run depcruise`: 0 modules cruised in both the host and webview graphs (installed TypeScript 7 is unsupported by dependency-cruiser) — environment limitation, unchanged from Checkpoints 3–5.
- `npm run test:coverage`: **red**, but for the same 5 pre-existing, unrelated files confirmed red at HEAD via stash-and-rerun: `test/backend/module-boundaries.test.ts` (3/7, already recorded), `test/backend/probe-tier-convention.test.ts`, `test/backend/reference-source-lute.test.ts`, `test/backend/vendored-licenses.test.ts`, `test/backend/manifest.test.ts` — 7 failing tests total, 4563 passing. None touch a Task 574 file.
- `npm run check:coverage-modules`: **could not run** — it depends on `coverage/coverage-summary.json`, which Vitest's v8 coverage provider does not write when any test in the run fails and `coverage.reportOnFailure` is not set (confirmed: setting `--coverage.reportOnFailure=true` produces the file). Since the aggregate `test:coverage` run is red on pre-existing, unrelated tests (above) regardless of this task, this gate has been unusable on this branch since before Task 574 started, not because of anything in Checkpoints 6–7. Targeted per-file coverage was used instead (see "Changed-line coverage" above) to verify Task 574's own changed lines, per the vmde-testing skill's changed-line-coverage recipe.

**Bundle/startup (reporting-only):** `media/dist/main.js` is 888,177 bytes / 867.36 KiB versus 877,928 bytes / 857.35 KiB at Task 573's close (`+10,249 bytes / +10.01 KiB`). Eager modules: 342 versus 339 (`+3`, matching the three new production modules this task added: `nav/source-block-index.ts`, `editing/details-source.ts`, `editing/details-selection-state.ts`). Largest eager module unchanged (29.8 KB, `vditor/src/ts/util/fixBrowserBehavior.ts`). Legacy budget scripts still report both bundle-size and startup-cost as exceeded (608 KB / 294-module ceilings); ceilings were not changed, per the queue's reporting-only policy.

### Checkpoint 7 feedback-path resolution (2026-09-26, Claude Opus 5.5)

The queue's routing for Checkpoints 6–7 sends a caret/undo/shared-state defect found at acceptance back through the §2a feedback path to Opus 5.5. This pass completed and verified an uncommitted fix already in the working tree; its authoring session is not recorded.

**Root cause, refined.** `snapshotPair()` anchors an exact transaction's `rendered` baseline at the first read. Checkpoint 6 removed the bubble's per-mutation snapshots, which had anchored early by accident. Edits that edit-sync does not own could then land before that first read and be paired with the pre-edit exact bytes. Two such edits exist: untrusted formatting input, and Vditor undo/redo, which changes the DOM without an input callback.

**Fix (`e0114c1a`, `bridge/edit-sync.ts`, `boot/finish-init.ts`, `boot/vditor-init.ts`).** The flag `editBeforeAnchor` is set by `schedule()`, `markUserInput()` and a new `markEditorChange()` while no baseline is anchored. `markEditorChange()` is called from the `installVditorHistoryCoupling` post callback before the history message reaches the host. When the flag is set, the first read anchors only if `rendered` equals the exact bytes or Lute's visual-mode projection of them (render, then serialize). Otherwise it revokes exact ownership as a later mismatch always did. SV and missing-Lute cases keep the previous anchoring, and a projection throw revokes. `postExact`/`reseed` clear the flag. `block-transform-command.ts` is unchanged.

**Separate improvement (`e47124d3`, `chrome/webview-context.ts`).** Restamping reconciled contexts by clearing and re-adding every `data-vscode-context` on each presentation class change. The shared index counts that churn as a DOM change. Contexts are now reconciled in place. Result: real-VS-Code IR keyboard phases drop from 1 index build to 0; WYSIWYG keyboard phases stay at 1, the owner-accepted Vditor keydown mutation. This file was outside the task's modify list. It is a separate commit, so the owner can drop it independently.

**Verification on the final candidate** (`--retries=0 --workers=1`, one build, serial real-VS-Code runs):
- Red check: with HEAD's `edit-sync.ts`, the 6 new edit-sync tests fail; they pass with the fix. Two more tests cover the projection-throw and no-projection branches. Focused units: 96/96 (edit-sync, finish-init, webview-context, undo-keybind, source-block-index, selection-bubble), plus 53/53 after the coverage tests.
- Real VS Code: `selection-bubble.spec.ts` 4/4, including the previously failing Bold → Undo → Redo → Save → Turn Into chain. `selection-performance.spec.ts` passes with OS XTEST input: keyboard at most 1 index build / 1 `getValue` per phase, 0 markers, cold open 1 `getValue`, cold after edit 1/1/0. IR cold-hover Lute entry points are identical to the 3 pre-fix runs (`VditorIRDOM2Md` 93, `Md2VditorIRDOM` 31, root 3), so the fix adds no cold-hover work.
- More real-VS-Code runs: `details-toolbar`, `block-handle` 13/13, `large-document-interaction` 3/3 and `list-normalize-history` 1/1 pass. `edit-propagation` passes 2/2. `block-transform` passes 6/7 (see the flake note below). Under Openbox and `VMDE_XTEST=1`, the OS-keyboard acceptance tests pass: `block-handle` Alt+Down/Up exact move history, and `large-document-interaction` Undo/Redo/save/reopen on the large copy.
- `block-transform.spec.ts` "native Turn Into QuickPick" is a **pre-existing intermittent failure**, not a regression. It failed 1 of 4 runs at HEAD and 1 of 8 in a `--repeat-each=8` trace run on the fix. A temporary webview trace, since removed, showed the second request accepted, with every validity check true, in the failing run exactly as in passing runs. So the native QuickPick sometimes fails to reappear after the options leave the webview. Possible follow-up, outside this task.
- Chromium: `selection-performance`, `selection-bubble`, `details` and `block-handle` pass 43/43.
- Cold-hover serializer target, measured three times with `large-document-interaction.spec.ts`, the Task 573 metric behind the 318 ms baseline: IR 1 `getValue`, `getValueMs` 126 / 126 / 120, **60% less** than the 318 ms median. WYSIWYG 74 / 77 / 77 vs 175 ms, **56% less**. Target ≥ 40% **met** on the serializer time. The whole cold hover (`coldMs` 875–982 IR) is not lower because of the resolver's out-of-scope fragment verification (H2).
- Changed-line coverage: all changed lines are covered except the SV arm of the projection's mode ternary (`edit-sync.ts` 375/381) and the unchanged-behavior SV stamping line (`webview-context.ts` 72).
- Typechecks: `typecheck` is clean. `typecheck:strict` has the 13 pre-existing diagnostics, none in changed files. Biome is clean on the changed files.
- Quality components (dependency audits omitted by owner policy): `lint:ci` has the same 4 pre-existing format errors plus 2 pre-existing warnings/infos. `knip` shows 9 unused exports and 1 unused type, none in changed files. `jscpd` is at 8.55% (threshold 8.8%). `depcruise` cruises 0 modules (TypeScript 7 unsupported). `test:coverage` has the same 7 pre-existing failures in 5 backend files, with 4576 passing. `check:coverage-modules` cannot run; see above.
- Bundle (reporting only): `main.js` is 888,821 B / 867.99 KiB, +644 B vs Checkpoint 7's 888,177 B and +10,893 B vs Task 573's close. Eager modules: 342 (unchanged). Legacy ceilings are still exceeded; no ceiling was changed.
...
| 7 feedback | `EditSync` edit-before-anchor proof (`e0114c1a`); webview-context in-place reconcile (`e47124d3`); closure record |

...
- [x] Reproduce and resolve premature closing: in the Task 563 real-VS-Code browser-input
...
- [x] Verify opening, grid/search interaction, Escape dismissal and the intended focus return
...
- [x] Integrate the regression with the new picker rather than adding a competing activation
...
- [x] **Search box:** focus it on opening; filter by emoji name and keywords, case-insensitively.
...
- [x] **Grid:** consistent tile/hit-target sizes, responsive column count, visible hover/focus,
...
- [x] **Grid background (Project Owner addition, 2026-09-08):** emoji slots have a
...
- [x] **Recently used (Project Owner addition, 2026-09-08):** show a labeled “Recently used”
...
- [x] **Categories:** group tiles under visible text headings using Unicode/CLDR group order
...
- [x] **Emoji 17.0:** provide the full recommended emoji repertoire for that version, including
...
- [x] Keep at most 24 distinct exact Unicode sequences, newest successful picker insertion
...
- [x] Keep the region visible when empty with “No recently used emoji yet.” Do not seed it
...
- [x] Persist the bounded list locally per VS Code profile across picker closure, document
...
- [x] Give the region an accessible heading and use the picker’s existing grid keyboard model.
...
- [x] Pin Unicode Emoji 17.0 data and a compatible, explicitly versioned CLDR annotation source.
...
- [x] Derive catalog completeness from the official fully-qualified recommended sequences;
...
- [x] Separate dataset coverage from font support. An Emoji 17.0 entry must be recognizable in
...
- [x] Insert the selected Unicode sequence as text, never an image URL, HTML asset or guessed
...
- [x] Keep runtime operation offline. Lazy-load catalog/artwork and avoid rendering the entire
...
- [x] Replace the current eight-item panel behind the existing Emoji control. Do not add another
...
- [x] Preserve the document selection before moving focus to search. Clicking or pressing Enter
...
- [x] Support source, IR and WYSIWYG, including inline/code-block text insertion. Keep mutations
...
- [x] Use accessible search, category and tile names, logical Tab navigation, arrow navigation
...
- [x] Keep existing typed-shortcode behavior intact. If sharing the new catalog with autocomplete
      is useful, do not expand this task into a separate shortcode/parser rewrite.

## Verification

- [x] Unit coverage for recents ordering, deduplication, the 24-entry cap, exact variants,
...
- [x] Chromium coverage for the recents region's position, empty/populated/filtered states,
...
- [x] Build first and run a focused real-VS-Code spec under xvfb for recent selection through
...
- [x] Catalog checks compare exact sequence sets against the pinned official data, validate
      categories/annotations and variants, and include representative additions from 17.0.
- [x] Unit coverage for search/ranking, aliases, no results, variant lookup and exact insertion
      strings; ensure sequences are never split by code-point or UTF-16 indexing mistakes.
- [x] Chromium coverage for categorized browsing, search/clear, scrolling, narrow widths,
      keyboard/variant selection, focus restoration, cancellation and one-step undo/redo.
- [x] Build first, then a focused real-VS-Code spec under xvfb opens the real toolbar picker,
...
- [x] Inspect light/dark screenshots against the supplied layout reference and verify fallback
      glyph rendering on a host without native 17.0 font coverage.
- [x] Record catalog/artwork size and startup/open/search cost; run applicable focused gates and
      the queue-authorized network-free validation tier before closure.
...
- [x] The equivalent WYSIWYG/SV real-VS-Code matrix now passes through the shared immutable source
      transaction. Each row replaces prose, inline code and a non-first fenced-code source range,
      verifies exact host bytes plus one undo/redo step after each insertion, then saves and reopens.
- [x] Fresh light/dark/live-switch/high-contrast/fallback screenshots, timing measurements, and a
      completed fresh VSIX archive validation are recorded in the final closure below.

## Astra-low matrix setup repair — 2026-09-08

- [x] The matrix now asserts the original value before switching, uses the documented 1500 ms
...
- [x] The repaired runner completed under escalated Xvfb. The original WYS prose failure was first
      reproduced twice (`🫪IR-PROSE` while `prose-wys` remained untouched), then the WYS row passed
      after the transaction correction; the final IR/WYS/SV matrix and the dedicated WYS code-source
      regression passed after a fresh build.

## Immutable Emoji source transaction — 2026-09-08

- [x] Picker activation now freezes serializer-derived numeric source offsets before focus leaves
      the editor. Later `mousedown`/click handlers cannot overwrite the pointerdown bookmark, and a
      focus-repair `selectionchange` cannot replace a retained directional selection with an
      unauthorized offset-zero caret. No live `Range` survives into mutation or undo checkpointing.
- [x] One guarded transaction validates editor/session/mode/generation/writable state plus exact and
      rendered snapshots synchronously, splices the complete Unicode sequence, brackets the
      mode-owned render with native undo checkpoints, suppresses delayed duplicate snapshots, posts
      exact history through the existing rewrap history authority, and returns a fresh keep-open
      bookmark. Input, accepted host updates (including same-content init), dismissal, mode handoff
      and disposal invalidate stale bookmarks.
- [x] Failed rendering restores source, selection, scroll and the active native undo metadata before
      reporting failure. Only an explicit successful result promotes in-session recents or posts the
      profile update; undo does not remove that successful usage history.
- [x] Red/green evidence: the new pure/source unit suite initially failed with the missing transaction
      and the Chromium early-capture regression inserted at document start. Final focused evidence is
      18 transaction unit tests; 129 affected unit tests across insertion/edit-sync/message routing,
      rewrap and WYS highlighting; 10 Chromium Emoji journeys; scoped Biome and all three type checks;
      fresh build; and focused real-VS-Code WYS code-source plus IR/WYS/SV source matrices.
- [x] Final closure resolved the remaining visual/fallback-font inspection, timing/startup
      measurement and packaged-asset validation without raising the existing shared ceilings.

## Final closure — 2026-09-08

- [x] The final real-VS-Code visual pass caught and corrected two Vditor toolbar cascade leaks:
      its invisible upload-input rule had made the search input absolute/near-transparent, and its
      open Emoji tooltip painted over the input. The picker now resets the search field only within
      its own header, keeps text-menu styling off the cloned panel, and suppresses only the open
      trigger tooltip. The focused spec asserts a usable static/opaque search field, non-overlap,
      hidden open-tooltip pseudo-elements, transparent fallback/catalog tiles, and the real
      high-contrast 3 px focus outline.
- [x] Fresh real-VS-Code evidence (one worker under Xvfb) passed the light → dark → high-contrast
      live switch with the picker open, direct narrow split placement, ordinary headings/menu
      semantics, source replacement/save/reopen/undo flows, and OS-keyboard Space opening/recent
      selection. The final light/dark/high-contrast screenshots were inspected; the `distorted
      face` fallback tile uses the bundled `VMDE Emoji 17` family and no opaque tile backplate.
- [x] Focused unit evidence: 27/27 across catalog, recents, picker filtering and immutable
      insertion; this includes the exact 3,944 fully-qualified Emoji 17.0 sequence set, CLDR 47
      provenance/checksums, variants and representative 17.0 additions. Focused Chromium picker
      coverage passed 8/8 in the grouped run; its one overflow-close failure passed in a fresh
      isolated no-retry reproduction, indicating test-run timing rather than a persisted picker
      regression.
- [x] Measured real-editor first picker opening at 611.3 ms and local filtered search at 141.6 ms
      on this Xvfb host. The catalog is 455,460 bytes and lazy-fetched on first open; fallback font
      is 10,673,480 bytes and loaded only as the picker's `@font-face`. The fresh build emitted
      `media/dist/main.js` at 700.6 kB (717,398 archived bytes) and CSS at 46.7 kB.
- [x] Fresh `artifacts/vmde-1.4.0.vsix` validated as production
      `Laicasaane.vmde` 1.4.0 (SHA-256
      `6bde09ee3416a9e038d92b7548bb1b51d5e03c2117b034de9fb34e86565a1c27`). Archive inspection
      confirms the catalog, fallback font/license and generated `main.js`/`main.css` are shipped.
- [x] The inherited startup and bundle gates remain deliberately unmodified and red:
      310/294 eager modules and 701/608 KiB `main.js`; the largest eager module remains 29.7 KiB,
      and all separately lazy engine assets remain under their ceilings. The queue explicitly
      prohibits broad quality/audit runs and arbitrary ceiling increases, so these are recorded
      shared-budget risks rather than a claim of aggregate quality.
